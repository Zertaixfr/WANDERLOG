...
const Globe3D = ({ travelerStatus, posts = [], trips = [] }) => {
  ...
  useEffect(() => {
    ...
    // Halo atmosphérique doré/chaud
    const haloGeometry = new THREE.SphereGeometry(globeRadius + 0.08, 64, 64)
    ...
    // Grille de méridiens subtile (style vintage)
    const gridGroup = new THREE.Group()
    const gridMat = new THREE.LineBasicMaterial({ color: 0xd4a044, transparent: true, opacity: 0.04 })
    // Méridiens
    for (let lng = -180; lng < 180; lng += 30) {
      const pts = []
      for (let lat = -90; lat <= 90; lat += 3) {
        pts.push(latLngToVector3(lat, lng, globeRadius + 0.003))
      }
